...
async function walkMarkdownFiles(directory, connectionId, prefix = '', rootName = directory.name, existingById = new Map()) {
  ...
  for await (const entry of directory.values()) {
    if (entry.kind === 'directory') {
      if (['.git', 'node_modules', 'build', 'dist', '.memoroid-backups'].includes(entry.name)) continue;
...
async function saveToOriginal() {
  if (!currentFileHandle) {
    downloadCurrentFile();
    showToast('В fallback сохранено как новый файл');
    return;
  }
  try {
...
function inline(text) {
  ...
  result = result.replace(/!\[([^\]]*)\]\(([^)\s]+)\)/g, (match, alt, source) => {
    if (!/^(?:javascript:|data:)/i.test(source)) {
  ...
  result = result.replace(/\u0000IMG\u0000([^\u0000]*)\u0000IMG\u0000([^\u0000]*)\u0000IMG\u0000/g,
    (match, source, alt) => `<img src="${escapeHtml(source)}" alt="${alt}" loading="lazy">`);
...
function selectDocument(name, source, metadata = {}) {
  ...
  currentFileName = name; currentMarkdown = source; currentPageId = metadata.pageId || createPageId(); rawMode = false; searchTerm = '';
  editorMode = false;
...
document.addEventListener('keydown', (e) => {
  if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'o') { e.preventDefault(); $('fileInput').click(); }
